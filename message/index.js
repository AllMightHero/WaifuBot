...
module.exports = msgHandler = async (bocchi = new Client(), message) => {
    ...
    .then(async (res) => {
        ...
        if (afsize >= 16.0 || asize.data.filesize.endsWith('GB')) {
            bocchi.reply(from, `Lo sentimos, para evitar prohibiciones de WhatsApp, el limite de envio de audio es de 16 MB, y esto tiene ${asize.data.filesize}.`, id)
        } else {
...
case 'ytmp4':
    ...
    break
            ...
            case 'stikergif':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif') {
                    ...
                    try {
                        const mediaData = await decryptMedia(message, uaOverride)
                        const videoBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
                        await bocchi.sendMp4AsSticker(from, videoBase64, { keepScale: true, fps: 10, startTime: `00:00:00.0`, endTime : `00:00:10.0`, loop: 0 }, { author: 'Otumaito', pack: 'Creado por WaifuBot' })
                    ...
                    try {
                        ...
                        await bocchi.sendMp4AsSticker(from, videoBase64, { keepScale: true, fps: 10, startTime: `00:00:00.0`, endTime : `00:00:10.0`, loop: 0 }, { author: 'powsitos.com', pack: 'Creado por CriSaBot' })
            ...
            case 'nuevolink':
                ...
                bocchi.sendTextWithMentions(from, `link de grupo actualizado pot @${sender.id.replace('@c.us', '')}`)
            break
            case 'linkgrupo':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
